...
- **Primary:** `tools/adm` (Unix) or `tools/adm.exe` (Windows) when the project has adm installed there (e.g. after `--install-adid-rules`). That executable is stable; if you edit the tool with adm and there is an error, you cannot run adm anymore and the toolchain breaksâ€”the copy in `tools/` avoids that.
- **Fallback:** `uv run adm` with the same subcommands when tools/adm is not present (e.g. in the adm repo before install).

**Rule:** Use `tools/adm` when presentâ€”same as AGENTS.md; avoids misunderstanding and toolchain break when editing the tool with adm.
...
| `--emit-boot-log [roots...]` | Runs pytest then verify-all; writes `boot_test.log` and verify reports. | One-shot â€œbootâ€ check: tests + verification. |
...
2. Run `tools/adm --template all` (or a specific template like `replace`, `overwrite`, `create`, `insert`, `delete`, `pattern-rule`, `refactor-replace-function`) â†’ creates a timestamped descriptor under `updates/`.
...
All mutations create backups and ledger entries; use `--rollback <file>` to undo a single file. Do not use git restore for one bad editâ€”use rollback.

**From now on, use this toolset:** template â†’ edit the descriptor file â†’ apply. **Use `tools/adm`** (or `tools/adm.exe` on Windows) when the project has itâ€”same as AGENTS.md; stable executable, toolchain stays intact if you edit the tool with adm and hit an error. Multiple backups per file are intentional and beneficial (rollback, traceability); the project stays manageable while saving time and giving a clear, testable way to use the tool across different areas.
